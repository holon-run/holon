...
fn principal_and_scope(state: &AppState) -> Result<(&'static str, String)> {
...
pub(crate) async fn brief_read_states(
    ...
    let (principal, scope) = match principal_and_scope(&state) {
        Ok(value) => value,
        Err(error) => return error_response(error).into_response(),
    };
    match state.host.runtime_db().brief_read_states(principal, &scope) {
