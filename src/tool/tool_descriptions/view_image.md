...
When the selected vision model uses an image-capable transport such as
OpenAI-compatible APIs or Anthropic Messages, `ViewImage` sends the image and
prompt to that model and returns a generated `visual_observation` alongside the
durable image metadata and selection diagnostics. If the selected model or
