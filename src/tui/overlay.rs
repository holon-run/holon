...
fn draw_help_overlay(frame: &mut Frame<'_>, scroll: u16) {
    ...
    let mut help_lines = [
        ...
        "  Ctrl+O starts overlay shortcuts from the main view",
        "  Ctrl+O H help, A agents, T tasks, S state",
        "  Ctrl+O C conversation, E events, M model, K selected-agent skills",
        "",
        "Quick Help",
        "  ? open this help when the composer is empty",
        "",
        "Scrolling",
        "  PgUp/PgDn scroll the active text view (Mac laptops: Fn+Up/Fn+Down)",
        "  Home/End jump to top/bottom (Mac laptops: Fn+Left/Fn+Right)",
        "",
        "Exit",
        "  Ctrl+C quit",
    ]
