...
use async_trait::async_trait;
    ...
    pub(super) fn context_config() -> ContextConfig {
        ContextConfig {
            recent_messages: 8,
            recent_briefs: 8,
    ...
    pub(super) async fn host_backed_test_runtime() -> (TempDir, RuntimeHost, RuntimeHandle) {
    ...
    pub(super) fn private_child_identity(agent_id: &str) -> AgentIdentityView {
        AgentIdentityView {
    ...
    pub(super) fn test_effective_prompt() -> EffectivePrompt {
        EffectivePrompt {
    ...
    pub(super) async fn bind_turn_to_work_item(runtime: &RuntimeHandle, work_item_id: &str) {
    ...
    pub(super) async fn seed_bound_work_item(
    ...
    pub(super) async fn mark_blocking_task(runtime: &RuntimeHandle, task_id: &str) {
        runtime
            .inner
            .storage
    ...
    pub(super) struct TruncatingProvider {
        pub(super) calls: Mutex<usize>,
    }

    pub(super) struct TimelineProvider;

    pub(super) struct OneToolThenTextProvider {
        pub(super) calls: Mutex<usize>,
    }
    ...
    pub(super) struct FailingTimelineProvider;

    pub(super) struct ToolCaptureProvider {
        pub(super) requests: Mutex<Vec<Vec<String>>>,
    }

    pub(super) struct TurnLocalCompactionProbeProvider {
        pub(super) calls: Mutex<usize>,
        pub(super) requests: Mutex<Vec<ProviderTurnRequest>>,
    }

    pub(super) struct BaselineOverBudgetProbeProvider {
        pub(super) calls: Mutex<usize>,
    }

    pub(super) struct ContextLengthExceededProvider;

    pub(super) struct SleepOnlyToolProvider {
        pub(super) calls: Mutex<usize>,
    }

    pub(super) struct DisallowedToolThenTextProvider {
        pub(super) calls: Mutex<usize>,
    }

    pub(super) struct MaxOutputMutationToolProvider {
        pub(super) calls: Mutex<usize>,
    }
    ...
    pub(super) struct StagnatingAfterVerificationProvider {
        pub(super) calls: Mutex<usize>,
    }

    pub(super) struct SkillReadProvider {
        pub(super) calls: Mutex<usize>,
    }
    ...
    impl CountingProvider {
        pub(super) async fn call_count(&self) -> usize {
