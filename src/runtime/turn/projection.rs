...
use super::{
    reminders::{build_delta_checkpoint_prompt, push_runtime_reminder_message},
    tool_summary::build_compacted_round_recap,
    TurnLocalCheckpointMode, TurnLocalCheckpointRequest, TurnLocalCheckpointState,
    COMPACTION_BOUNDARY_FULL_PROGRESS_CHECKPOINT_PROMPT, CONTINUATION_BUDGET_SAFETY_MARGIN_TOKENS,
    DEGRADED_ROUND_MINIMUM_CONTENT_CHARS, DEGRADED_ROUND_PROVENANCE_MARKER, MIN_EXACT_TAIL_ROUNDS,
};

pub(super) fn truncate_preview(text: &str, limit: usize) -> String {
...
pub(super) fn build_checkpoint_resume_round(
    ...
    let continuation_text = super::CHECKPOINT_RESUME_PROMPT.to_string();
    TurnRoundRecord {
        round,
...
pub(super) fn append_follow_up_user_texts(round: &mut TurnRoundRecord, texts: Vec<String>) {
    if texts.is_empty() {
        return;
    }
...
pub(super) fn render_operator_interjection_text(message: &crate::types::MessageEnvelope) -> String {
    use super::OPERATOR_INTERJECTION_HEADER;
    use crate::runtime::message_dispatch::message_text;
    format!(
...
#[derive(Debug, Clone)]
pub(super) struct TurnRoundRecord {
    pub(super) round: usize,
    pub(super) assistant_blocks: Vec<ModelBlock>,
    pub(super) text_blocks: Vec<String>,
    pub(super) tool_calls: Vec<crate::tool::ToolCall>,
    pub(super) tool_results: Vec<ToolResultBlock>,
    pub(super) tool_result_envelopes: Vec<crate::tool::spec::ToolResultEnvelope>,
    pub(super) follow_up_user_texts: Vec<String>,
...
pub(super) fn build_round_estimated_tokens(
