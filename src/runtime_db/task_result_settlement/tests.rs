...
fn admission_is_bounded_and_leaves_overflow_pending() {
    ...
    let admitted = db
        ...
        .admit_unsettled("agent-a", "work-a", "activation-1", Utc::now())
        .unwrap();
    ...
    let overflow = db
        .task_result_settlements()
        .admit_unsettled("agent-a", "work-a", "activation-2", Utc::now())
        .unwrap();
...
fn admitted_result_can_be_rebound_after_restart() {
    ...
    let lock_path = dir.path().join("state/runtime.lock");
    {
        ...
        let admitted = db
            .task_result_settlements()
            .admit_unsettled("agent-a", "work-a", "activation-before", Utc::now())
            .unwrap();
    ...
    let rebound = reopened
        .task_result_settlements()
        .admit_unsettled("agent-a", "work-a", "activation-after", Utc::now())
        .unwrap();
...
fn active_admission_is_not_stolen_by_another_activation() {
    ...
    let admitted = db
        .task_result_settlements()
        .admit_unsettled("agent-a", "work-a", "activation-active", Utc::now())
        .unwrap();
    assert_eq!(admitted.len(), 1);
    db.transaction(|tx| {
        tx.execute(
    ...
    assert!(db
        .task_result_settlements()
        .admit_unsettled("agent-a", "work-a", "activation-other", Utc::now())
        .unwrap()
        .is_empty());
    let latest = db
        .task_result_settlements()
...
fn settlement_rolls_back_with_its_enclosing_terminal_transaction() {
    ...
    insert(&db, &record);
    db.task_result_settlements()
        .admit_unsettled("agent-a", "work-a", "activation-rollback", Utc::now())
        .unwrap();
