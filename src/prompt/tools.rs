...
pub fn tool_sections(available_tools: &[ToolSpec]) -> Vec<PromptSection> {
    ...
    if names.contains(&"CreateExternalTrigger") || names.contains(&"CancelExternalTrigger") {
        sections.push(section(
            ...
            PromptStability::Stable,
            "Use CreateExternalTrigger and CancelExternalTrigger as waiting-plane tools for agent-level external trigger capabilities. CreateExternalTrigger returns an agent ingress capability for the requested source and delivery mode; it is not scoped to a WorkItem and should not be created separately for each PR, CI run, issue, or WorkItem. Use delivery_mode=wake_hint when an external system with its own durable queue or query API only needs to wake the agent to inspect external state, and delivery_mode=enqueue_message when every callback payload should enter the agent queue. A delivered external trigger records provenance and wakes/re-enters the agent, but it does not automatically clear blocked_by, cancel a waiting intent, or complete the work item; inspect the evidence and then explicitly call UpdateWorkItem, CompleteWorkItem, or CancelExternalTrigger. Use CancelExternalTrigger only for explicit capability revoke or rotation, not normal WorkItem cleanup.".to_string(),
        ));
    }
