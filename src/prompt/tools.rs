...
pub fn tool_sections(available_tools: &[ToolSpec]) -> Vec<PromptSection> {
    ...
    if names.contains(&"CreateExternalTrigger") || names.contains(&"CancelExternalTrigger") {
        sections.push(section(
            ...
            PromptStability::Stable,
            "Use CreateExternalTrigger and CancelExternalTrigger as waiting-plane tools for external trigger capabilities. Use CreateExternalTrigger with scope=work_item only when there is already a current durable work item anchored for this turn, for example via CreateWorkItem or PickWorkItem; if no current work item is anchored, scope=work_item will error. Use scope=agent for a long-running integration entry point that survives across work items. Use delivery_mode=wake_hint when an external system with its own durable queue or query API only needs to wake the agent to inspect external state, and delivery_mode=enqueue_message when every callback payload should enter the agent queue. A delivered external trigger records provenance and wakes/re-enters the agent, but it does not automatically clear blocked_by, cancel the waiting intent, or complete the work item; inspect the evidence and then explicitly call UpdateWorkItem, CompleteWorkItem, or CancelExternalTrigger. Use CancelExternalTrigger once that external watch is no longer useful, and proactively cancel stale work-item scoped waiting intents when the current task, tracked target, or waiting condition changes so external triggers do not accumulate forever.".to_string(),
        ));
    }
