...
const ROUTES: &[RouteSpec] = &[
    ...
    aide_route("get", "/control/agents/{agent_id}/status", "controlAgentStatus", "control", "Agent status (control)", "Return the AgentSummary for any agent through the control plane. Under the current simplified trust model, local/operator access is trusted, so this can return private child agents.", None, AuthKind::Control),
