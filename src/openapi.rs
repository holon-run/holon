...
const ROUTES: &[RouteSpec] = &[
    ...
    route("get", "/agents/{agent_id}/events", "agentEvents", "events", "Agent event page", "Return a bounded page of projected runtime events. Query parameters: before_seq, after_seq, limit, order, projection. Both operator (default) and local_debug projections include the full raw event payload; local_debug additionally requires control auth.", None, AuthKind::RemoteAccess),
