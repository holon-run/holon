...
(`type` and non-empty `summary`) strict. The `schema` label is compatibility
metadata supplied by the provider: missing, unknown, or older labels must not
invalidate an otherwise usable observation whose structure is valid. Holon
normalizes accepted observations to its canonical `visual_observation.v1`
schema. Auxiliary observation fields are best-effort sensor data and must not
invalidate an otherwise usable observation solely because a provider ignored
its strict response schema.
