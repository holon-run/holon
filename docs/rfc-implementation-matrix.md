...
| `rfc-agent-lifecycle-control-posture` | [agent-lifecycle-control-posture](rfcs/agent-lifecycle-control-posture.md) | Proposed | `ControlAction`, `AgentStatus`, `SchedulerDecisionExecutor`, lifecycle/control paths | `cargo test` | New RFC. Target direction is `Start`/`Stop`, removal of ambiguous `Pause`/`Resume`, and scheduler-executor ownership of status-like posture writes. | 012, 039, 044 |
...
| `rfc-operator-interjection-safe-points` | [operator-interjection-safe-points](rfcs/operator-interjection-safe-points.md) | Proposed | `scheduler::is_operator_interjection_message`, `TurnExecution::drain_operator_interjections`, turn safe-point branches, provider request lowering | focused runtime/provider projection tests | New RFC. Defines that ordinary operator interjection appends input and must not cancel accepted pending tool calls or create empty provider-visible messages. | 014, 021, 027 |
...
| `rfc-provider-capability-registry` | [provider-capability-registry](rfcs/provider-capability-registry.md) | Implemented | `src/provider/registry.rs`, built-in provider config, model discovery and catalog registration | provider registry, discovery and catalog contract tests; `cargo test --lib` | Static built-in definitions and closed transport families remain separate from runtime/custom provider config. | 032 |
...
| `rfc-runtime-scheduler-contract` | [runtime-scheduler-contract](rfcs/runtime-scheduler-contract.md) | Partial | `SchedulerProjection`, `decide_next_action`, `SchedulerDecisionExecutor`, task transition reducer, work-queue idempotency keys, scheduler replay fixtures | `cargo test scheduler --quiet`; focused continuation, wake-hint, scheduler-continuation, and operator-interjection tests | Main gap-closing PRs are landed. Remaining follow-up is reducer cleanup around turn-loop safe-point interjection, lifecycle/control posture ownership tracked by [agent-lifecycle-control-posture](rfcs/agent-lifecycle-control-posture.md), and fallback duplicate evidence. See [runtime-scheduler-contract](rfc-implementation-notes/runtime-scheduler-contract.md). | 012, 039, 044 |
...
| `rfc-tui-vim-composer-mode` | [tui-vim-composer-mode](rfcs/tui-vim-composer-mode.md) | Implemented | TUI composer edit mode, `/vim` slash command | `cargo test vim_ -- --test-threads=1` | Session-local TUI ergonomics only; no runtime contract changes. | 058 |
...
| `rfc-work-item-centered-agent-runtime` | [work-item-centered-agent-runtime](rfcs/work-item-centered-agent-runtime.md) | Proposed | WorkItem tools, scheduler work-queue projection, waiting intents, briefs, context compaction | `cargo test` | New umbrella RFC for WorkItem-centered long-running agent work; lower-level RFCs should be updated as concrete schema/scheduler/memory changes land. | 015, 034, 044, 052 |
