...
| `POST` | `/control/agents/:agent_id/work-items/:work_item_id/complete` | `{ report_text, authority_class? }` | `WorkItemRecord` | Experimental | Atomically completes an open WorkItem with `report_text` as its canonical result brief; also finalizes an existing legacy `completing` record. Cancel/delete remains out of scope. |
