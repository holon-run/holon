...
| `POST` | `/control/agents/:agent_id/work-items/:work_item_id/complete` | `{ report_text, authority_class? }` | `WorkItemRecord` | Experimental | Records completion intent and returns the WorkItem in `completing`; it becomes `completed` only after `report_text` is persisted and atomically bound as the result brief. Cancel/delete remains out of scope. |
