---
title: 持久 Agent 工作流
summary: "端到端的持久 Agent 故事：创建 Agent、启动长任务、熬过断连、等待事件，并交付最终简报。"
order: 5
---

# 持久 Agent 工作流

Holon 与一次性 CLI 工具最根本的区别是**持久性**：Agent 能跨越终端会话持续
工作、熬过断连、等待外部事件，并在条件满足时恢复。本指南完整走一遍这个生命
周期。

## 持久性体现在哪？

Holon 的 Agent 会话独立于任何客户端连接，持久保存这些内容：

- **Agent 身份和 home** — 每个 Agent 拥有 `~/.holon/agents/<name>/`，内含
  自己的 `AGENTS.md`、skills 和长期记忆
- **工作队列** — 工作项、排队消息，以及 blocked/waiting 状态
- **任务生命周期** — 正在运行的命令和子 Agent 委派在客户端断开后继续
- **休眠/唤醒状态** — Agent 空闲时休眠，遇到事件或操作者输入时唤醒

daemon 让运行时保持存活。客户端（TUI、HTTP、一次性 CLI）连接、提交工作、
查看进度、断开，工作照常继续。
