...
Holon 把 Agent 执行当作运行时系统来处理。单次模型回合很重要，但只是一部分。
...
工作项让 Holon 能跨回合恢复工作、查看进度，或把未完成的工作移交给其他 Agent。
...
2. Agent 在同一回合调用 `CompleteWorkItem`
...
完成报告取代了自由格式的手写总结。它与工作项生命周期绑定，但不要求模型回合
必须结束。`CompleteWorkItem` 调用就是该工作项的生命周期边界：运行时捕获对应
报告并完成该工作项。如果没有后续动作，运行时可以直接停止，不需要模型再重复
一遍同样的报告作为第二份最终简报。如果该回合还有后续 assistant 输出、工具
调用或其他 WorkItem 完成操作，这些延续动作属于同一回合，不会覆盖已经提升的
完成报告。
...
简报和任务结果会关联回产生它们的运行时回合。这样 Holon 就能在唤醒和任务结果
...
- [上下文连续性](/zh-CN/concepts/context-continuity.md) — Holon 如何在回合之间保持模型可见的连续性
- [记忆系统](/zh-CN/concepts/memory.md) — Holon 如何在回合之间保持连续性
