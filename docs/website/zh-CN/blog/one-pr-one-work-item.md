...
> **中文审阅稿 · 未发布。** 案例来自 Holon 仓库自身的 `holon-reviewer`；案例记录采集于 2026 年 9 月 10 日，文末统计核验于 9 月 12 日。
...
我们核对了 Holon 仓库自身的 `holon-reviewer` 在 **2026 年 9 月 7 日至 11 日这五个完整 UTC 日**的审阅记录。按独立 PR 去重，并以首次审阅记录进入窗口为准，不按 PR 创建时间或最终是否合并挑选样本。

| 统计项 | 数量 |
| --- | ---: |
| 窗口内有 Reviewer 审阅记录的 PR | 61 |
| 这些 PR 在同一窗口内完成合并 | 61 |
| 样本内尚未合并的 PR | 0 |

这些 PR 上，共享 GitHub 账号在窗口内留下了 **74 次正式 review、42 条跟进评论**。这是账号层面的公开活动数，不能全部算作 `holon-reviewer` 的操作次数；61 个 PR 已合并，也不等于 61 次合并都由它执行。

这组数据补充了开头的单条案例：同一个 reviewer 在连续几天里参与了数十条 PR 的审阅与跟进。不过，“首审无阻塞后合并”和“根据反馈修复、复查后合并”还没有逐条完成归因，不能从 review 次数直接推算。它也不是质量测试：全部合并，不代表没有漏掉问题，更不能据此推断换一个仓库能节省多少时间。

## 案例与统计来源

文中的 PR #2854 使用了完整工作计划及 GitHub 提交、审阅、检查和合并快照；PR #2856 的迟到通知处理来自结果简报，GitHub 快照确认它已合并。本次未重新执行案例中的测试，也未独立验证订阅清理结果。

统计窗口为 2026-09-07 00:00 UTC（含）至 2026-09-12 00:00 UTC（不含）。Reviewer 的本地记录用于确定 PR 范围，GitHub 快照用于核对公开审阅、评论及合并时间；74 次 review 和 42 条评论均限定于上述样本与窗口，不含 bot 账号活动。逐 PR 脱敏核验表、可重算数据和限制说明保存在仓库的 `docs/marketing/evidence/reviewer-2026-09-07--11/`。
