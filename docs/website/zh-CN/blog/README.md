...
- [Holon 是什么：让多个 Agent 在你的工作环境里持续做事](./what-is-holon.md)
  从交代任务、查看进展到等待后继续，认识 Holon 这套供多个 Agent 持续工作的本地工作台，以及它在远程开发和团队协作中的用法。中文审阅稿，未发布。
...
- [Holon 的 WorkItem 架构：把工作状态从对话中分离](./why-work-items.md)
  从 token 河流上的一条船说起，拆解 WorkItem 如何连接 LLM、调度与恢复上下文，并用 reviewer 交错跟进两个 PR 解释等待和工作连续性。中文审阅稿，未发布。
  <!-- mdorigin:index kind=article -->
...
- [不止审一次代码：用 Holon 搭建持续跟进 PR 的 Reviewer](./one-pr-one-work-item.md)
  从两处 reviewer 的真实记录，看审阅判断如何随提交和 CI 更新，以及何时应把决定交还维护者。
  <!-- mdorigin:index kind=article -->
