...
> **中文审阅稿 · 未发布。** 案例已省略内部编号与环境标识，公开范围待确认。
...
如果你也想从团队共享的运行环境开始，可以参考[远程访问与服务部署指南](../../guides/remote-access.md)（英文），了解如何让成员连接同一个 Holon 服务；再用 [WorkItem 指南](../../guides/work-items.md)组织需要持续跟进的工作。先选一件团队里反复发生、交接边界清楚的事，比一开始就安排一整套 Agent 岗位更容易检验这种协作是否有用。
