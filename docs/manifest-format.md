...
- **Artifact Inventory:** Lists all generated output files (skill-defined artifacts)
- **Runtime Metadata:** Captures agent version, mode, and engine information for debugging and reproducibility
- **Skill-First Contract:** The only Holon-standardized artifact; all other outputs are skill-defined
- **Type Safety Contract:** Ensures Go runner can parse TypeScript-generated outputs without type drift

## Schema

The canonical JSON Schema is maintained at [`schemas/run.manifest.schema.json`](../schemas/run.manifest.schema.json).
...
The `artifacts` array lists **all outputs** generated by the skill execution:
- **Code skills**: May include `diff.patch`, `summary.md`, `evidence/`, etc.
- **GitHub skills**: May include `pr-fix.json`, `publish-intent.json`, etc.
- **Custom skills**: Any skill-defined outputs

Skills SHOULD declare all artifacts they generate in this array to provide a complete inventory for automation and orchestration.
...
1. **Single Source of Truth:** [`schemas/run.manifest.schema.json`](../schemas/run.manifest.schema.json) defines the canonical format
...
```bash
npx ajv validate \
  -s schemas/run.manifest.schema.json \
