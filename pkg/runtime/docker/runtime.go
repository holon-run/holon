...
func (r *Runtime) RunHolon(ctx context.Context, cfg *ContainerConfig) (string, error) {
	...
	// In skill-first mode, skills are responsible for all IO including publish.
	// manifest.json is no longer a blocking requirement since skills may succeed
	// without generating it. Log a warning for debugging but don't fail execution.
