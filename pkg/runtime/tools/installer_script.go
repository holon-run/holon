...
func BuildInstallScript() string {
	...
	b.WriteString("        bash ca-certificates curl git jq ripgrep findutils sed gawk tar gzip unzip python3 make patch gnupg\n")
	b.WriteString("      rm -rf /var/lib/apt/lists/*\n")
	b.WriteString("      ;;\n")
	b.WriteString("    dnf)\n")
	b.WriteString("      dnf install -y bash ca-certificates curl git jq ripgrep findutils sed gawk tar gzip unzip python3 make patch gnupg2\n")
	b.WriteString("      dnf clean all\n")
	b.WriteString("      rm -rf /var/cache/dnf\n")
	b.WriteString("      ;;\n")
	b.WriteString("    yum)\n")
	b.WriteString("      yum install -y epel-release || true\n")
	b.WriteString("      yum install -y bash ca-certificates curl git jq ripgrep findutils sed gawk tar gzip unzip python3 make patch gnupg2\n")
	b.WriteString("      yum clean all\n")
	b.WriteString("      rm -rf /var/cache/yum\n")
	b.WriteString("      ;;\n")
	b.WriteString("  esac\n")
	b.WriteString("}\n")
	b.WriteString("node_major() {\n")
	b.WriteString("  if ! command -v node >/dev/null 2>&1; then echo 0; return; fi\n")
	b.WriteString("  node -p \"process.versions.node.split('.')[0]\" 2>/dev/null || echo 0\n")
	b.WriteString("}\n")
	b.WriteString("ensure_node_runtime() {\n")
	b.WriteString("  MAJOR=$(node_major)\n")
	b.WriteString("  if [ \"$MAJOR\" -ge 24 ] && command -v npm >/dev/null 2>&1; then return; fi\n")
	b.WriteString("  case \"$PM\" in\n")
	b.WriteString("    apt)\n")
	b.WriteString("      install -m 0755 -d /etc/apt/keyrings\n")
	b.WriteString("      curl -fsSL https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key | gpg --dearmor -o /etc/apt/keyrings/nodesource.gpg\n")
	b.WriteString("      chmod a+r /etc/apt/keyrings/nodesource.gpg\n")
	b.WriteString("      echo \"deb [signed-by=/etc/apt/keyrings/nodesource.gpg] https://deb.nodesource.com/node_24.x nodistro main\" > /etc/apt/sources.list.d/nodesource.list\n")
	b.WriteString("      retry_cmd apt-get update -o Acquire::Retries=3\n")
	b.WriteString("      retry_cmd env DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends -o Acquire::Retries=3 nodejs\n")
	b.WriteString("      rm -rf /var/lib/apt/lists/*\n")
	b.WriteString("      ;;\n")
	b.WriteString("    dnf)\n")
	b.WriteString("      curl -fsSL https://rpm.nodesource.com/setup_24.x | bash -\n")
	b.WriteString("      dnf install -y nodejs\n")
	b.WriteString("      dnf clean all\n")
	b.WriteString("      rm -rf /var/cache/dnf\n")
	b.WriteString("      ;;\n")
	b.WriteString("    yum)\n")
	b.WriteString("      curl -fsSL https://rpm.nodesource.com/setup_24.x | bash -\n")
	b.WriteString("      yum install -y nodejs\n")
	b.WriteString("      yum clean all\n")
	b.WriteString("      rm -rf /var/cache/yum\n")
	b.WriteString("      ;;\n")
	b.WriteString("  esac\n")
