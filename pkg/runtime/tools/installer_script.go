...
func BuildInstallScript() string {
	...
	b.WriteString("      ;;\n")
	b.WriteString("    yum)\n")
	b.WriteString("      yum install -y bash ca-certificates curl git jq findutils sed gawk tar gzip unzip python3 nodejs npm make patch gnupg2\n")
	b.WriteString("      ;;\n")
	b.WriteString("  esac\n")
	b.WriteString("}\n")
	...
	b.WriteString("      apt-get update\n")
	b.WriteString("      DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends gh\n")
	b.WriteString("      rm -rf /var/lib/apt/lists/*\n")
	b.WriteString("      ;;\n")
	b.WriteString("    dnf)\n")
	...
	b.WriteString("  ARCH=$(arch_map)\n")
	b.WriteString("  curl -fsSL \"https://github.com/mikefarah/yq/releases/latest/download/yq_linux_${ARCH}\" -o /usr/local/bin/yq\n")
	...
	b.WriteString("  curl -fsSL \"https://github.com/sharkdp/fd/releases/latest/download/fd-v10.2.0-${ARCH}-unknown-linux-musl.tar.gz\" -o \"$FDTAR\"\n")
