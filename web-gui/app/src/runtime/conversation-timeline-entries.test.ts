...
it("interleaves inputs using durable activity keys without repeating represented operator activities", () => {
  ...
  const activities: ConversationActivity[] = [1, 2, 3, 4, 5, 6].map((seq) => {
    const id = seq % 2 ? `a${seq}` : `operator:m${seq}`;
    return { kind: seq % 2 ? "assistant" : "operator", id,
      key: { event_seq: seq, activity_id: id }, revision: 1, summary: "Progress" };
  });
  expect(conversationTimelineEntries(inputs, activities).map((entry) => entry.id))
    .toEqual(["a1", "m2", "a3", "m4", "a5", "operator:m6"]);
  ...
  expect(conversationTimelineEntries(inputs, activities.slice(-1)).map((entry) => entry.id))
    .toEqual(["m2", "m4", "operator:m6"]);
});
